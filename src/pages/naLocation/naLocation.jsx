...
export default class NaLocation extends Component {
  ...
  getNaRecords = async (page,pageSize) => {
    ...
    const result = await reqNaLocations(page,pageSize)
    // 在请求完成后, 隐藏loading
  ...
  searchNaLocation = async (page,pageSize) => {
    ...
    if (searchName||searchStatus) {
      result = await reqNaLocation(searchName,searchStatus,page,pageSize)
  ...
  render() {
    ...
    return (
      <Card title={title} extra={extra} >
          ...
          pagination={{defaultPageSize: 5,
            ...
            onChange: (page, pageSize) => {
              if (!isSearch){
                ...
                this.searchNaLocation(page,pageSize)
              }
            }}}
          size="middle"
        />
        ...
        <Modal
          title={naLocation.id ? '修改检测点' :'添加检测点'}
          visible={isShow}
